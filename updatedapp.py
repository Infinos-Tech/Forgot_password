   ...
   if request.method=="POST":
       ...
       if Useruthentication.query.filter_by(username=un.hexdigest()).first():
           flash("username taken","danger")
            ...
            NewUser = Useruthentication(username=un.hexdigest(),userpassword=up.hexdigest(),
            phno=phno,
            mailid=email,
            rectym=datetime.now())
            db.session.add(NewUser)
...
def Get_OTP():
    if request.method=="POST":
        ...
        if result is not None:
            ...
            mail.send(msg) 
            flash(f"hello A otp has been sent you registered email id {result.mailid}","success")
